...
class AasTemplateJsonFormatter:
    ...
    def serialize(
        self, data: DataModel
    ) -> Dict[Literal["assetAdministrationShells", "submodels"], List[str]]:
        """
        ...
        return json.loads(
            basyx.aas.adapter.json.object_store_to_json(basyx_dict_obj_store)
        )

    def deserialize(
        self, data: Dict[Literal["assetAdministrationShells", "submodels"], List[str]]
    ) -> DataModel:
        """
        ...
        for key, items in data.items():
            if key == "assetAdministrationShells":
                for aas_item in items:
                    aas = json.loads(
                        json.dumps(aas_item),
                        cls=basyx.aas.adapter.json.AASFromJsonDecoder,
                    )
                    object_store.add(aas)
            elif key == "submodels":
                for submodel_item in items:
                    submodel = json.loads(
                        json.dumps(submodel_item),
                        cls=basyx.aas.adapter.json.AASFromJsonDecoder,
                    )
...
class AasJsonFormatter:
    ...
    def deserialize(
        self,
        data: Dict[Literal["assetAdministrationShells", "submodels"], List[str]],
        types: List[type[Submodel]] | List[type[AAS]],
    ) -> DataModel:
        """
        Deserialize the specific format of the formater to a DataModel object.
