...
def get_submodel(client: TestClient, example_aas_instance: ValidAAS):
    ...
    response = client.get(
        url=f"/{class_name}/{example_aas_instance.id}/example_submodel/"
    )
...
def update_submodel(client: TestClient, example_aas_instance: ValidAAS):
    ...
    response = client.put(
        url=f"/{class_name}/{example_aas_instance.id}/example_submodel", content=data
    )

    assert response.status_code == 200
    updated_aas = client.get(url=f"/{class_name}/{example_aas_instance.id}/")
    assert updated_aas.status_code == 200
    assert updated_aas.json()["example_submodel"]["list_attribute"] == [
        "new_list_element"
    ]

    updated_sm = client.get(
        url=f"/{class_name}/{example_aas_instance.id}/example_submodel"
    )
...
def delete_submodel(client: TestClient, example_aas_instance: ValidAAS):
    ...
    response = client.delete(
        url=f"/{class_name}/{example_aas_instance.id}/optional_submodel"
    )
    assert response.status_code == 200
...
def post_submodel(client: TestClient, example_aas_instance: ValidAAS):
    ...
    response = client.post(
        url=f"/{class_name}/{example_aas_instance.id}/optional_submodel", content=data
    )
    assert response.status_code == 200
