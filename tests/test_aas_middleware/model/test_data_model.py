...
def test_minimal_example_with_AAS(example_aas: ValidAAS):
    ...
    assert (
        len(data_model._key_ids_models) == 8
    )  # 1 aas, 4 submodels, 3 submodel element collections
    ...
    example_submodel_ids = set(
        [get_id_with_patch(model) for model in example_submodel_models]
    )
    assert example_submodel_ids == {
        "example_submodel_id",
        "example_submodel_for_union_id",
        "example_optional_submodel_id",
    }
    ...
    assert data_model.get_referencing_models(example_aas.example_submodel) == [
        example_aas
    ]
    submodel_references = data_model.get_referenced_models(example_aas.example_submodel)
    submodel_references_id_set = set(
        [get_id_with_patch(reference) for reference in submodel_references]
    )
    assert submodel_references_id_set == {
        "example_submodel_element_collection_id",
        "simple_submodel_element_collection_id",
        "example_submodel_element_collection_for_union_id",
    }
    assert (
        data_model.get_referenced_models(
            example_aas.example_submodel.submodel_element_collection_attribute_simple
        )
        == []
    )


def test_normalize_complex_example(example_aas: ValidAAS, referenced_aas_1: ValidAAS):
    ...
    assert (
        id(example_aas_list[0]) != id(example_aas_list[1])
        and id(example_aas_list[1]) != id(example_aas_list[2])
        and id(example_aas_list[0]) != id(example_aas_list[2])
    )
    assert (
        id(referenced_aas_1_list[0].example_submodel)
        != id(referenced_aas_1_list[1].example_submodel)
        and id(referenced_aas_1_list[1].example_submodel)
        != id(referenced_aas_1_list[2].example_submodel)
        and id(referenced_aas_1_list[0].example_submodel)
        != id(referenced_aas_1_list[2].example_submodel)
    )

    normalized_list = normalize_identifiables(example_aas_list)
    assert (
        id(normalized_list[0]) == id(normalized_list[1])
        and id(normalized_list[1]) == id(normalized_list[2])
        and id(normalized_list[0]) == id(normalized_list[2])
    )

    normalized_referenced_aas_1_list = normalize_identifiables(referenced_aas_1_list)
    assert (
        id(normalized_referenced_aas_1_list[0].example_submodel)
        == id(normalized_referenced_aas_1_list[1].example_submodel)
        and id(normalized_referenced_aas_1_list[1].example_submodel)
        == id(normalized_referenced_aas_1_list[2].example_submodel)
        and id(normalized_referenced_aas_1_list[0].example_submodel)
        == id(normalized_referenced_aas_1_list[2].example_submodel)
    )
...
def test_more_complex_example(
    ...
    assert data_model.get_model("example_submodel_id") == example_aas.example_submodel
    assert (
        data_model.get_model("example_basemodel_with_id") == example_basemodel_with_id
    )
    assert data_model.get_model("example_object_with_id") == example_object_with_id
    assert (
    ...
    submodel_2_model_ids = set(
        [get_id_with_patch(model) for model in submodel_2_models]
    )
    ...
    referencing_model_ids = set(
        [get_id_with_patch(model) for model in referencing_models]
    )
    assert referencing_model_ids == {
        "example_submodel_with_reference_components_id",
        "example_submodel_with_id_reference_components_id",
        "example_submodel_with_product_association_id",
    }

    referenced_models = data_model.get_referenced_models(
        example_submodel_with_reference
    )
    assert len(referenced_models) == 2
    referenced_model_ids = set(
        [get_id_with_patch(model) for model in referenced_models]
    )
...
def test_remove_and_data_model_as_base_model():
    ...
    try:
        registry.remove(
            "John Doe"
        )  # removal not possible because Alice links to John doe with a reference
        assert False
    except ValueError as e:
        pass
    registry.remove("Alice")
    assert set(registry.get_contained_ids()) == {
        "Jane Doe",
        "Bob",
        "John Smith",
        "Jane Smith",
